# 3IO-SFC MODEL (BASED ON MODEL PC + 3-INDUSTRY IO TABLES)
...
for (j in 1:nScenarios){
  ...
  for (i in 2:nPeriods){
    ...
    for (iterations in 1:nIterations){
      ...
      # Shock 1: higher government spending
      if (i>=50 && j==2){
          if(g[j,i-1]>=23){g[j,i]=23}else{g[j,i]=g[j,i-1]+0.1} # Option 1: growing over time
          #g[j,i]=23 # Option 2: one-off shock
        }
...
# PLOT RESULTS FOR THE BASELINE SCENARIO ####
...
# PLOT RESULTS FOR THE ALTERNATIVE SCENARIO ####
...
# Add title
title("Alternative scenario: increase in government spending", outer = TRUE)

# Figure 2: disposable income and consumption
plot(period,yd[2,48:tspan],type="l", col=2, lwd=2, lty=1, font.main=1,cex.main=1,
     main="Figure 2. Disposable income and consumption",
     ylab = '\u00A3',xlab = '',ylim = range(min(cons[2,48:tspan]*p_c[2,48:tspan]),max(yd[2,48:tspan])),
     cex.axis=1,cex.lab=1)
lines(period,cons[2,48:tspan]*p_c[2,48:tspan],type="l",lwd=2,lty=1,col="purple1")
legend("right",c("Disposable income","Nom. consumption"),  bty = "n",
       cex=1, lty=c(1,1), lwd=c(2,2), col = c(2,"purple1"), box.lty=0)
abline(h=yd[1,nPeriods],col=1,lty=3)

# Figure 3: Final demand by industry 
plot(period,d[2,48:tspan,1]*p[2,48:tspan,1],type="l",lwd=2,col="springgreen4",
     main="Figure 3. Final demand by industry (nominal)",
     font.main=1,cex.main=1,
     ylab = '£',xlab = '',ylim=range(max(p[2,48:tspan,]*d[2,48:tspan,]),min(p[2,48:tspan,]*d[2,48:tspan,])))
lines(period,p[2,48:tspan,2]*d[2,48:tspan,2],type="l",lwd=2,col="orangered",lty=1)
lines(period,p[2,48:tspan,3]*d[2,48:tspan,3],type="l",lwd=2,col="dodgerblue3",lty=1)
legend("right",c("Agriculture","Manufacturing","Services"),  bty = 'n', cex=1, lty=c(1,1,1), lwd=c(2,2,2), col = c("springgreen4","orangered","dodgerblue3"), box.lty=0)

# Figure 4: Gross outputs by industry 
plot(x[2,48:tspan,1],type="l",lwd=2,col="springgreen4",
     main="Figure 4. Gross output by industry (real)",
     font.main=1,cex.main=1,
     ylab = 'Index',xlab = '',ylim=range(min(x[2,48:tspan,]),max(x[2,48:tspan,])))
lines(x[2,48:tspan,2],type="l",lwd=2,col="orangered",lty=1)
lines(x[2,48:tspan,3],type="l",lwd=2,col="dodgerblue3",lty=1)
legend("right",c("Agriculture","Manufacturing","Services"),  bty = 'n', cex=1, lty=c(1,1,1), lwd=c(2,2,2), col = c("springgreen4","orangered","dodgerblue3"), box.lty=0)

# Figure 5: Unit prices by industry 
plot(period,p[2,48:tspan,1],type="l",lwd=2,col="springgreen4",
     main="Figure 5. Unit prices by industry",
     font.main=1,cex.main=1,
     ylab = '£',xlab = '',ylim=range(min(p[2,48:tspan,]),max(p[2,48:tspan,])))
lines(period,p[2,48:tspan,2],type="l",lwd=2,col="orangered",lty=1)
lines(period,p[2,48:tspan,3],type="l",lwd=2,col="dodgerblue3",lty=1)
legend("topright",c("Agriculture","Manufacturing","Services"),  bty = 'n', cex=1, lty=c(1,1,1), lwd=c(2,2,2), col = c("springgreen4","orangered","dodgerblue3"), box.lty=0)
